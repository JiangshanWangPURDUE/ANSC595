...
colnames(metadata)[2] = "Animal ID"
colnames(metadata)[3] = "site"
levels(metadata$"site")

metadata$"Body Site" = factor(metadata$"site", c("Duodenum", "Jejunum", "Ileum", "Cecum", "Ascending_colon", "Transverse_colon", "Descending_colon", "Capsule"))
...
physeq <- qza_to_phyloseq(
  ...
  "taxonomy.qza",
  metadata = "SampleSeq_metadata2.xls.txt"
)
...
for(ml in my_level){
  ...
  physeq_meta_filtered$"Body_Site" = factor(physeq_meta_filtered$site, c("Duodenum", "Jejunum", "Ileum", "Cecum", "Ascending_colon", "Transverse_colon", "Descending_colon", "Capsule"))
  
  # Plot 
  ggplot(physeq_meta_filtered, aes(x = Body_Site, y = Abundance.average, fill = get(ml))) + 
  ...
  ggsave(paste0("R output/", ml, "BarPlot_", my_column, ".png"), height = 5, width = 6)
}
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ site)
...
alpha = 0.05
my_contrast = c("site", "Jejunum", "Cecum") 
...
ggsave(paste0("R output/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 3, width = 10)
