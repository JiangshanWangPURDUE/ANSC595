...
str(meta)
colnames(meta)[2] <- "Animal ID"
colnames(meta)[3] <- "Site"
...
aov.evenness.site = aov(pielou_evenness ~ Site, data=meta)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.site)
...
meta$site.ord = factor(meta$Site, c("Duodenum", "Jejunum", "Ileum", "Cecum", "Ascending_colon", "Transverse_colon", "Descending_colon", "Capsule"))
levels(meta$site.ord)

#Plot
boxplot(pielou_evenness ~ site.ord, data=meta, ylab="Peilou evenness")

evenness <- ggplot(meta, aes(site.ord, pielou_evenness)) + 
  geom_boxplot(aes(color = site.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("R output/evenness.png", evenness, height = 3, width = 5)
...
evenness <- ggplot(meta, aes(site.ord, pielou_evenness)) + 
  ...
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("R output/evenness_no color.png", evenness, height = 3, width = 5)
...
evenness_summary <- meta %>% # the names of the new data frame and the data frame to be summarised
  group_by(site.ord) %>%   # the grouping variable
...
evenness_se <- ggplot(evenness_summary, aes(site.ord, mean_evenness, fill = site.ord)) + 
  geom_col() + 
...
ggsave("output/evenness_se.png", evenness_se, height = 3, width = 5)
...
shannon <- ggplot(meta, aes(site.ord, shannon_entropy)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Shannon", x = "") 
ggsave("R output/shannon.png", shannon, height = 3, width = 5)



observedfeatures <- ggplot(meta, aes(site.ord, observed_features)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggsave("R output/observedfeature_no color.png", observedfeatures, height = 3, width = 5)
...
#Plot
boxplot(faith_pd ~ site.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd <- ggplot(meta, aes(site.ord, faith_pd)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggsave("R output/pd_no color.png", faith_pd, height = 3, width = 5)
